...
function toTime(data: Date = new Date(), type: String = 'yyyy-mm-dd') {
  let nowData: Date | String = new Date(data);
  switch (type) {
    case 'yyyy-mm-dd':
      nowData = `${nowData.getFullYear()}年-${
        nowData.getMonth() + 1
      }月-${nowData.getDate()}日`;
      break;
    case 'yyyy-mm':
      nowData = `${nowData.getFullYear()}年-${nowData.getMonth() + 1}月`;
      break;
  }
  return nowData;
}

export { isLeapYear, remainTime, isSameDay, toTime };
